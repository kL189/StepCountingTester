...
public class PlotViewer {
    ...
    public static void main(String[] args) {
        ...
        ArrayList<Double> smoothOut = counter.smoothOut(mags);
        ...
        for (int i = 0; i < mags.size(); i++) {
            ...
            plt.plot(3, i, DefaultStepCounter.getMagThreshold(mags)).strokeColor("black").strokeWeight(2).style(".");
        }
        System.out.println(DefaultStepCounter.getMagThreshold(mags));

        for (int i = 0; i < peakIndexes.size(); i++) {
            plt.plot(1, peakIndexes.get(i), peakValues.get(i)).strokeColor("blue").strokeWeight(5).style(".");
        }

        for (int i = 0; i < smoothOut.size(); i++) {
            plt.plot(2, i, smoothOut.get(i)).strokeColor("green").strokeWeight(2).style("-");
        }
